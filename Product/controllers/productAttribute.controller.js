...
const model = "product attribute";
const Model = "Product Attribute";
const ProductAttributeController = {
  getAll: async (req, res, next) => {
    try {
      const filter = req.query;
      const list = await ProductAttributeService.getAll(filter, "");
      if (!list) {
  ...
  getById: async (req, res, next) => {
    try {
      const { id } = req.params;
      const object = await ProductAttributeService.getById(id);
      if (!object) {
  ...
  update: async (req, res, next) => {
    try {
      const data = req.body;
      const { id } = req.params;
      const object = await ProductAttributeService.update(id, data);
      if (!object) {
        return next(createError.BadRequest(Model + " not found"));
      }
      res.json({
  ...
  delete: async (req, res, next) => {
    try {
      const { id } = req.params;
      const object = await ProductAttributeService.delete(id);
      if (!object) {
        return next(createError.BadRequest(Model + " not found"));
      }
      res.json({
