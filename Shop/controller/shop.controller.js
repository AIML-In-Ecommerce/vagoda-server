...
const ShopController = {
  ...
  getShopByIdList: async (req, res, next) => {
    try {
      ...
      const listOfShops = await ShopService.getByIds(
        shopIds,
        useShopDetail,
        useDesign
      );
  ...
  getByAccountId: async (req, res, next) => {
    try {
      ...
      const shopInfo = await ShopService.getByAccountId(
        accountId,
        useShopDetail,
        useDesign
      );
  ...
  getShopBySelection: async (req, res, next) => {
    try {
      ...
      if (shopId != undefined) {
        ...
        object = await ShopService.getByAccountId(
          accountId,
          useShopDetail,
          useDesign
        );
  ...
  addImageLinks: async (req, res, next) => {
    try {
      ...
      if (!Array.isArray(imageUrls)) {
        return next(
          createError.BadRequest("Image URLs must be an array of strings")
        );
      }
      ...
      for (let url of imageUrls) {
        if (typeof url !== "string") {
          return next(
            createError.BadRequest("Each image URL must be a string")
          );
        }
      }
      ...
      const updatedImageCollection = await ShopService.addImageLinks(
        shop,
        cloudinaryUrls
      );
  ...
  removeImageLinks: async (req, res, next) => {
    try {
      ...
      const updatedImageCollection = await ShopService.removeImageLinks(
        shop,
        imageUrls
      );
